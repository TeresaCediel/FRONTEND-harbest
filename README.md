...
- Primero usar ```npm run build``` para crear la carpeta ```dist``` que es la carpeta que se usará en render para desplegar el frontend.
- Para iniciar el Frontend usar el comando ```npm run dev```, cada vez que see guarda el código automáticamente se muestran en la página web.
...
En desarrollo. Versión inicial funcional con login, home y perfil.
